...
path_origin = 'results/baselines/mins-dkitty/mins_2023-09-07_06-21-21'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 mins-robust dkitty --local-dir results/BOSS/mins-robust/dkitty --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
