...
path_origin = 'results/baselines/gradient-ascent-mean-ensemble-tf-bind-10/gradient-ascent-mean-ensemble_2023-09-22_05-22-28'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 gradient-ascent-mean-ensemble-robust tf-bind-10 --local-dir results/BOSS/gradient-ascent-mean-ensemble-robust/tf-bind-10 --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
