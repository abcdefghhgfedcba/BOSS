      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 gradient-ascent-mean-ensemble-robust tf-bind-8 --local-dir results/SIRO/gradient-ascent-mean-ensemble-robust/tf-bind-8 --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
