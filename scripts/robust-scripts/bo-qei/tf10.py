...
path_origin = 'results/baselines/bo-qei-tf-bind-10/bo_qei_2023-08-28_13-20-10'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 bo-qei-robust tf-bind-10 --local-dir results/BOSS/bo-qei-robust/tf-bind-10 --cpus 5 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
