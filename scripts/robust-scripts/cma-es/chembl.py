...
path_origin = 'results/baselines/cma-es-chembl/cma-es_2023-08-09_04-21-20'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 cma-es-robust chembl --local-dir results/BOSS/cma-es-robust/chembl --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
