...
path_origin = 'results/baselines/cma-es-superconductor/cma-es_2023-07-19_08-22-21'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 cma-es-robust superconductor --local-dir results/BOSS/cma-es-robust/superconductor --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
