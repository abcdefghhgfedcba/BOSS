...
path_origin = 'results/baselines/gradient-ascent-tf-bind-10/gradient-ascent_2023-09-22_04-22-28'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 gradient-ascent-robust tf-bind-10 --local-dir results/BOSS/gradient-ascent-robust/tf-bind-10 --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
