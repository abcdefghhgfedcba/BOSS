...
path_origin = 'results/baselines/gradient-ascent-dkitty/gradient-ascent_2023-08-17_06-21-21'
      ...
      for omega_sigma_init in [1e-3]:
            for folder in os.listdir(path_origin):
                if os.path.isdir(os.path.join(path_origin, folder)):
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES=0 gradient-ascent-robust dkitty --local-dir results/BOSS/gradient-ascent-robust/dkitty --cpus 2 \
                        --gpus 1 \
                        --num-parallel 1 \
                        --num-samples 1 \
                        --omega_mu_init 0. \
